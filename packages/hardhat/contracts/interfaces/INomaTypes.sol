...
interface INomaTypes {
    ...
    event PaymentSettled(uint256 indexed paymentId, uint256 indexed leaseId, uint256 amount, string settlementChain);

    /// @notice Emitted when reputation is updated
    event ReputationUpdated(address indexed tenant, uint256 newScore, ReputationTier newTier);

    /// @notice Emitted when yield is routed
    event YieldRouted(uint256 indexed leaseId, uint256 amount, string strategy);

    /// @notice Emitted for AI agent trigger points
    event AIAgentTrigger(string indexed triggerType, uint256 indexed leaseId, bytes data);
}
